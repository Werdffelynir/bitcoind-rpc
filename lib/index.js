...
function generateRPCMethods(constructor, apiCalls, rpc) {
  ...
  function createRPCMethod(methodName, argMap) {
    return function() {
      ...
      // fixed query parameters for getblockhashes
      var _arguments = [].slice.call(arguments, 0);
      if (methodName === 'getblockhashes' && _arguments.length === 4) {
        _arguments.splice(2, 1);
      }
      ...
      if (this.batchedCalls) {
        this.batchedCalls.push({
          jsonrpc: '2.0',
          method: methodName,
          params: slice(_arguments),
          id: getRandomId()
        });
      } else {
        rpc.call(this, {
          method: methodName,
          params: slice(_arguments, 0, _arguments.length - 1),
          id: getRandomId()
